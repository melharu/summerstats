...
#Load in the data
data <- read.csv("~/ucsb_GIT_R/summerstats/week3/processed data/Week 3 Example Data PROCESSED.csv")
...
p3<-cor.test(data$agreea,data$guilt)
...
plot3<-ggplot(data=data,
              aes(x=agreea,
                  y=guilt))+
  labs(x="Partcipant Agreeableness",
       y="Guilt")+
  theme_gray(base_size=20)+
  geom_point()+
...
p4<-cor.test(data$nuero,data$guilt)

#Output the result
p4

#Test predictions 3 and 4 in a regression:
guiltReg<-lm(guilt~nuero+agreea,data=data)
summary(guiltReg)
...
plot5.1 <- ggplot(data=dataMelt,
  ...
  theme_gray(base_size=20)+
  scale_x_discrete(labels = c("Parnter", "self"))+
...
p7<-cor.test(data$agreea,data$postShare)
...
Prediction 8 was not supported. There was no relationship between nueroticism and generosity:
```{r p8}

#Run the correlation test for prediction 8
p8<-cor.test(data$nuero,data$postShare)

#Output the result
p8

#Test predictions 7 and 8 in a regression:
genReg<-lm(postShare~nuero+agreea,data=data)
summary(genReg)
